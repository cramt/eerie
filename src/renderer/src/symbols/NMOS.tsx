...
export const nmosPinPositions: Record<string, { x: number; y: number }> = {
  gate: { x: -30, y: 0 },
  drain: { x: 20, y: -30 },
  source: { x: 20, y: 30 },
  body: { x: 30, y: 0 },
...
export default function NMOS({ label, color, textColor }: Props) {
  return (
    <Group>
      ...
      <Line points={[-6, 10, 10, 10]} stroke={color} strokeWidth={2} />
      <Arrow points={[10, 10, 20, 10]} stroke={color} strokeWidth={2} fill={color} pointerLength={5} pointerWidth={4} />
      <Line points={[20, 10, 20, 30]} stroke={color} strokeWidth={2} />
    </Group>
  )
}
