...
export const pnpPinPositions: Record<string, { x: number; y: number }> = {
  base: { x: -30, y: 0 },
  collector: { x: 20, y: 30 },
  emitter: { x: 20, y: -30 },
}
...
export default function PNP({ label, color, textColor }: Props) {
  return (
    <Group>
      <Line points={[-30, 0, -6, 0]} stroke={color} strokeWidth={2} />
      <Line points={[-6, -16, -6, 16]} stroke={color} strokeWidth={2.5} />
      <Arrow points={[20, -30, -6, -8]} stroke={color} strokeWidth={2} fill={color} pointerLength={6} pointerWidth={5} />
